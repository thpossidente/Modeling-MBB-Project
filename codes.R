...
# The dataset has 20 classes, representing 20 separate objects/stimuli.
NUM_CLASSES <- 20L

# The objects/stimuli are represented by random binary strings of 5 digits.
OBJECT_CODE <- 5L
...
#
inference <- function(codes, hidden1_units, hidden2_units) {
  
  # Hidden 1
  with(tf$name_scope('hidden1'), {
    weights <- tf$Variable(
      tf$truncated_normal(shape(OBJECT_CODE, hidden1_units),
                          stddev = 1.0 / sqrt(OBJECT_CODE)),
      name = 'weights'
    )
    biases <- tf$Variable(tf$zeros(shape(hidden1_units),
                                   name = 'biases'))
    hidden1 <- tf$nn$relu(tf$matmul(codes, weights) + biases)
  })
